...
class DominoRestAPI {
	...
	@Get("/pedidos/estado") // TODO: no encuentro la forma de hacer el URL correcto
	def getPedidoByState(String estado) {
		response.contentType = ContentType.APPLICATION_JSON
		val matchedPedidos = this.dominoPizza.pedidos.filter [ c |
			c.estado.nombre.replaceAll("[^A-Za-z]+", "").toLowerCase == estado
		].toList
